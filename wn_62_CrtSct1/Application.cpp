		...
		case WM_COMMAND:
		{
			switch(LOWORD(wParam))
			{
				case ID_THREAD_CREATE:
				{
					...
					m_hTh[0] = (HANDLE)_beginthreadex(NULL, 0
						, (unsigned (__stdcall *) (void *))ThreadFunc
						, (void*)this
						, 0, (unsigned*)&m_dTh[0]);


					m_hTh[1] = (HANDLE)_beginthreadex(NULL, 0
						, (unsigned (__stdcall *) (void *))ThreadFunc
						, (void*)this
						, 0, (unsigned*)&m_dTh[1]);
				...
				case ID_THREAD_DESTROY:
				{
					if(m_hTh[0])
					{
						SuspendThread(m_hTh[0]);
						CloseHandle(m_hTh[0]);
					}

					if(m_hTh[1])
					{
						SuspendThread(m_hTh[1]);
						CloseHandle(m_hTh[1]);
					}
...
DWORD WINAPI ThreadFunc(void* pParam)
{
	...
	DWORD dThId = GetCurrentThreadId();
	...
	while(nCnt<100)
	{
		...
		sprintf(sBuf, "%ld %d", dThId, pApp->m_nValue);
