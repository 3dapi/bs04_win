...
INT CALLBACK TimerProc(HWND, UINT, UINT, DWORD);
...
INT CALLBACK TimerProc(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	HDC hdc;
	int i;
	hdc=GetDC(hWnd);
	...
	for (i=0;i<100;i++)
	{
		SetPixel(hdc,rand()%1024, rand()%768,
		RGB( INT(rand()%256), INT(rand()%256),INT(rand()%256) ));
	}
	ReleaseDC(hWnd, hdc);
